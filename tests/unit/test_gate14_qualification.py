...
def test_gate14_policy_and_real_project_pass() -> None:
    report = Gate14Qualification().collect(
        project_root=ROOT,
        candidate_id="openinfra-0.34.24-unit",
        source_commit="a" * 40,
    ...
    assert report.release_version == "0.34.24"
    assert report.metrics == Gate14Metrics(667, 35, 584, 48, 48, 83, 0, 0)
...
def test_gate14_report_serialization_and_atomic_write(
    ...
    report = Gate14Report(
        ...
        Gate14Metrics(667, 35, 584, 48, 48, 83, 0, 0),
...
def test_gate14_policy_rejects_invalid_contracts(tmp_path: Path) -> None:
    ...
    valid = {
        ...
        "expected_metrics": Gate14Metrics(667, 35, 584, 48, 48, 83, 0, 0).as_dict(),
    }
