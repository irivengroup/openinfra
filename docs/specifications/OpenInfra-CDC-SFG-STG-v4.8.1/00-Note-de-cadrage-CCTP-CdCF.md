...
OpenInfra doit être une plateforme open source de référence d’infrastructure, couvrant Source of Truth, DCIM, ITAM, Discovery, Dependency Mapping et IPAM Enterprise++, sans fonction ITSM intégrée.
...
Les scopes applicatifs `lite/all-in-one`, `pro/server`, `pro/web`, `enterprise/server` et `enterprise/web` doivent créer lors de l'installation un compte système `openinfra` et un filesystem LVM dédié monté sur `/opt/openinfra/`. Le scope `enterprise/agent` crée le compte/service requis mais ne crée pas de filesystem LVM applicatif ; il est installé directement sous `/opt/openinfra/`. Les valeurs par défaut sont `rootvg`, `openinfra_lv` et `2GB`.
