...
def _bootstrap(app: object, token: str, roles: tuple[str, ...] = ("ri:operator",)) -> None:
    app.security_service.bootstrap_token(  # type: ignore[attr-defined]
        BootstrapTokenCommand(
            tenant_id="default",
            actor="pytest",
            subject="ri-quality-admin",
            roles=roles,
            token=token,
        )
    )
...
def test_ri_quality_certifies_complete_authoritative_device(tmp_path: Path) -> None:
    app = ApplicationFactory().create_json_application(tmp_path / "state.json")
    token = "r" * 40
    _bootstrap(app, token, ("ri:governance-admin",))
    ...
    app.source_of_truth_service.upsert_object(
        UpsertSourceObjectCommand(
            ...
            actor="pytest",
            admin_token=token,
            key="device/ri-quality-001",
            kind="device",
            display_name="RI Quality 001",
    ...
    report = app.ressources_inventory_quality_service.evaluate_object(
        EvaluateRiObjectQualityCommand("default", token, "device/ri-quality-001")
    )
    summary = app.ressources_inventory_quality_service.summarize(
        RiQualitySummaryCommand("default", token, limit=10, kind="device")
    )
...
def test_ri_quality_rejects_incomplete_and_warns_non_authoritative_source(tmp_path: Path) -> None:
    app = ApplicationFactory().create_json_application(tmp_path / "state.json")
    token = "s" * 40
    _bootstrap(app, token, ("ri:governance-admin",))
    app.source_governance_service.create_rule(
    ...
    app.source_of_truth_service.upsert_object(
        UpsertSourceObjectCommand(
            ...
            actor="pytest",
            admin_token=token,
            key="device/ri-quality-002",
            kind="device",
            display_name="RI Quality 002",
    ...
    report = app.ressources_inventory_quality_service.evaluate_object(
        EvaluateRiObjectQualityCommand("default", token, "device/ri-quality-002")
    )
...
def test_ri_quality_requires_dedicated_read_permission(tmp_path: Path) -> None:
    app = ApplicationFactory().create_json_application(tmp_path / "state.json")
    writer_token = "t" * 40
    denied_token = "u" * 40
    _bootstrap(app, writer_token, ("ri:operator",))
    ...
    app.source_of_truth_service.upsert_object(
        UpsertSourceObjectCommand(
            ...
            key="device/ri-quality-003",
            kind="device",
            display_name="RI Quality 003",
    ...
    with pytest.raises(AccessDeniedError):
        app.ressources_inventory_quality_service.evaluate_object(
            EvaluateRiObjectQualityCommand("default", denied_token, "device/ri-quality-003")
        )


def test_cli_ri_quality_commands_and_sot_compatibility_alias(
    ...
    assert report["key"] == "device/ri-quality-cli"
