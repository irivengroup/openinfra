...
function Dashboard() {
  ...
  const filteredModules = MODULES.map((module) => ({ ...module, operations: module.operations.filter((operation) => `${module.label} ${operation.label}`.toLowerCase().includes(filter.toLowerCase())) }));
  ...
  return <div className="openinfra-shell">
    <header>
      ...
      <div className="px-3 py-2 border-bottom mb-3"><div className="container-fluid d-flex flex-wrap justify-content-center"><form className="col-12 col-lg-auto mb-2 mb-lg-0 me-lg-auto" role="search" onSubmit={(event) => event.preventDefault()}><input type="search" className="form-control openinfra-search" placeholder="Search OpenInfra operations..." aria-label="Search" value={filter} onChange={(event) => setFilter(event.target.value)} /></form><div className="text-end"><button type="button" className="btn btn-light text-dark me-2" onClick={() => chooseOperation(MODULES[5], MODULES[5].operations[0])}>Login</button><button type="button" className="btn btn-primary" onClick={() => chooseOperation(MODULES[5], MODULES[5].operations[0])}>Sign-up</button></div></div></div>
    </header>
