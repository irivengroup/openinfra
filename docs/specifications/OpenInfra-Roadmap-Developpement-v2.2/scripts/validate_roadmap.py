...
if len(phases) < 22:
    raise SystemExit(f"Expected at least 22 phases, got {len(phases)}")
if len(epics) < 131:
    raise SystemExit(f"Expected at least 131 epics, got {len(epics)}")
if len(gates) < 11:
    raise SystemExit(f"Expected at least 11 gates, got {len(gates)}")
if len(tests) < 106:
    raise SystemExit(f"Expected at least 106 tests, got {len(tests)}")
if len(alignment) < 20:
...
print(
    f"OK: roadmap v2.2 validated with {len(phases)} phases, {len(epics)} epics, {len(gates)} gates, {len(tests)} tests"
)
