...
Scalabilité horizontale API/workers, lectures via réplicas, partitionnement, hot/warm/cold, sharding applicatif possible tenant/site/VRF.
...
## Scale-out applicatif v4.9.0

Les API/BFF sont répliqués horizontalement sans session locale. La capacité s’augmente par réplica avant d’augmenter sans limite les workers d’un processus. La pagination curseur, le streaming et les workers évitent les charges mémoire proportionnelles aux volumes. Au-delà des capacités d’un cluster PostgreSQL vertical, les données append-only sont partitionnées temporellement et externalisées ; Citus ou un découpage cellulaire n’est adopté qu’après benchmark et plan de migration.
