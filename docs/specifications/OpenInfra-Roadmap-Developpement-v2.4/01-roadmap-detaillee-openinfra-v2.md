...
# Addendum 2.1.0 — Priorité haute performance Pro/Entreprise

## P19 — Socle runtime haute performance

La release 0.30.0 livre EPIC-1901 à EPIC-1905 : ASGI API/Web, pools PostgreSQL et HTTP, streaming, gates d’architecture et réalignement CDC/roadmap. Le runtime legacy reste disponible uniquement pour rollback contrôlé.

## P20 — Scale-out données et frontend

La phase suivante industrialise PgBouncer et le routage lecture, la pagination curseur, l’outbox et les workers spécialisés, puis le découpage frontend par domaine. Chaque incrément conserve la compatibilité REST/CLI/OpenAPI et dispose d’un benchmark p95/p99.

## Ordre d’exécution

1. Stabiliser 0.30.0 et mesurer ASGI/pools.
2. Déployer PgBouncer et métriques de saturation.
3. Introduire read replicas et cohérence lecture-après-écriture.
4. Migrer les collections les plus volumineuses vers des curseurs.
5. Sortir les traitements longs vers outbox/workers.
6. Découper le frontend et virtualiser les vues massives.
7. Valider endurance, chaos et failover avant promotion Enterprise Scale-out.
...
### Avancement v0.29.22 — titlebar dashboard concise

- La titlebar `Dashboard` gagne un espacement vertical responsive autour du titre et du sous-titre.
...
## Incrément v0.29.50 — P08 administration éditions et quotas
...
### Validation v0.29.72 — DCIM dépendances CRUD topologiques

La livraison v0.29.72 ajoute `TST-P10-DCIM-DEPENDENCY-CRUD` pour verrouiller la gestion explicite des bâtiments, étages, salles et zones DCIM. L'incrément renforce la phase P10 sans casser `define-room`, qui reste une opération composite compatible.
...
- Sites DCIM : rue, code postal, email et téléphone exposés en UI/CLI/API.
- Organisations ITAM : code postal et téléphone ajoutés au modèle et aux interfaces.
- Partenaires ITAM : code postal ajouté au modèle et aux interfaces.
- Pays : affichage nom seul, valeur alpha2.
- Migration : `0036_site_organization_addresses.sql`.
...
## Addendum 2.2.0 — P21 Kubernetes & Cloud-native

Après la promotion GATE-09 du socle Enterprise Scale-out, P21 complète les exigences `REQ-00469` et `REQ-00470` du CDC 4.9.0. La phase commence par EPIC-2101 : snapshots Kubernetes immuables, multi-cluster et auditables, puis graphe déterministe `cluster → namespace → workload/pod/service → node → VM → hyperviseur → serveur → rack → salle/site`.

Les incréments suivants couvrent les expositions et dépendances réseau, la corrélation images/SBOM/certificats/références de secrets, la dérive GitOps et la capacité par cluster/namespace. Aucun secret en clair n’est admis dans l’inventaire et aucune correction GitOps n’est appliquée silencieusement.

**Release :** REL-11 — Kubernetes & Cloud-native  
**Jalon :** M13  
**Gate :** GATE-10  
**Version d’ouverture :** OpenInfra 0.33.0 / EPIC-2101

## P23 — Licence runtime offline Pro/Entreprise

P23 est un lot prioritaire transverse livré par REL-13. Il couvre la chaîne de confiance Ed25519, la persistance JSON/PostgreSQL/Oracle, le quota transactionnel, l’enforcement CLI/API, l’installation offline, les notifications et GATE-12. Les détails exécutables sont définis dans `18-plan-licence-runtime-offline.md`.
