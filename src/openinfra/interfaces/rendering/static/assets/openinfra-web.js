import { OpenInfraI18n, localizeOpenInfraCatalog } from "./openinfra-i18n.js?v=0.34.21";
...
async function ensureManagementRegistryLoaded() {
  if (!managementRegistryPromise) {
    managementRegistryPromise = import("./management/resources.js?v=0.34.21").then((loaded) => {
...
} from "./openinfra-form-fields.js?v=0.34.21";
import { OPENINFRA_DOMAIN_LOADERS, OPENINFRA_MODULES, OPENINFRA_SIDEBAR_CONTEXTS } from "./openinfra-domain-manifest.js?v=0.34.21";
import { OpenInfraQueryCache } from "./openinfra-query-cache.js?v=0.34.21";
import { OpenInfraVirtualList } from "./openinfra-virtual-list.js?v=0.34.21";
import { installOpenInfraWebVitals } from "./openinfra-web-vitals.js?v=0.34.21";


class OpenInfraApiError extends Error {
  constructor(message, status) {
    super(message);
    this.name = "OpenInfraApiError";
...
class OpenInfraApiClient {
  ...
  async request(operation, payload) {
    ...
    if (operation.binaryUpload) {
      ...
      const upload = fileField ? payload[fileField.name] : null;
      if (!upload?.file) {
        throw new Error(this.i18n?.t("requiredField", { field: fileField?.label || "Fichier source" }) || "Missing required upload file");
      }
      headers["Content-Type"] = upload.media_type || "application/octet-stream";
      headers["X-OpenInfra-Filename"] = encodeURIComponent(upload.filename || "dataset");
      body = upload.file;
    } else if (operation.method !== "GET") {
      headers["Content-Type"] = "application/json";
      body = JSON.stringify(this.buildBody(operation, payload));
    }
    ...
    if (!response.ok) {
      ...
      throw new OpenInfraApiError(message, response.status);
    }
    return data;
  }
...
class OpenInfraDashboard {
  ...
  async loadSearchIndex() {
    ...
    const loaded = await import("./openinfra-search-index.js?v=0.34.21");
  ...
  sidebarOperationGroups(module, operations) {
    ...
    const groups = configuredGroups.map((group) => {
      const requestedIds = group.operationIds || operations.filter(({ id }) => id.startsWith(group.operationIdPrefix)).map(({ id }) => id);
      const existingIds = requestedIds.filter((id) => byId.has(id));
      ...
      <div class="container-fluid">
        <div class="row">
          ...
          <main id="openinfra-main-content" class="col-xl-10 ms-sm-auto openinfra-main" tabindex="-1">
            ...
            ${error ? `<div class="alert ${error.accessDenied ? "alert-danger openinfra-access-denied" : "alert-warning"} openinfra-error-summary" role="alert" tabindex="-1"${error.status ? ` data-http-status="${this.escape(error.status)}"` : ""}>${this.escape(error.message)}</div>` : ""}
  ...
  renderOperationPanel(operation, result) {
    ...
    return `<div class="row g-4">
      ...
      <aside class="col-12 col-xxl-4" aria-labelledby="openinfra-result-title">
        ...
        ${this.renderOperationResult(operation, result)}
  ...
  renderOperationResult(operation, result) {
    if (!result) {
      return "";
    }
    const custom = this.moduleForOperation(operation)?.renderResult?.(operation, result, this);
    if (custom) return custom;
    if (!String(operation?.id || "").startsWith("graph-")) {
      return "";
    }
  ...
  renderField(rawField) {
    ...
    if (field.type === "file") {
      ...
      return `<label${visibility} class="col-12 form-label">${this.escape(this.i18n.label(field.label || field.name))}${requiredText}<input class="form-control" type="file"${common}${accept}${capture}${required}><span class="form-text">${this.escape(fileHelpText(field))}</span></label>`;
    }
    ...
    try {
      ...
      for (const input of form.querySelectorAll("[data-field]")) {
        ...
        if (input.type === "file") {
          ...
          payload[input.dataset.field] = file
            ? this.state.selected.binaryUpload
              ? { file, filename: file.name, media_type: file.type }
              : await this.filePayload(file)
            : undefined;
        } else {
