# Migration vers le contrat RSOT canonique — OpenInfra 0.34.24

OpenInfra 0.34.24 retire définitivement les alias publics ITRM, RI et SOT. Le seul contrat pris en charge est **RSOT — Ressource Source of Truth**. Les fonctions métier sont conservées ; seuls les noms d’entrée publics et les imports Python historiques sont supprimés.
...
4. Corriger toutes les occurrences avant le déploiement 0.34.24.

## Remplacements obligatoires

| Ancien contrat | Contrat 0.34.24 |
| --- | --- |
...
openinfra-gate13 --project-root . --candidate-id openinfra-0.34.24-local --source-commit 0000000000000000000000000000000000000000 --output artifacts/gate13/report.json --enforce
```

## Rollback

Le rollback applicatif nécessite la réinstallation du paquet 0.34.5 correspondant au schéma déjà sauvegardé. Ne réintroduisez pas d’alias dans 0.34.24. Restaurez les configurations et automatismes sauvegardés, puis vérifiez les routes et permissions de la version restaurée avant réouverture du service.
