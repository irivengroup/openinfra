...
PYTHONPATH=src python -m openinfra.interfaces.cli rsot upsert-object --data "$tmpdir/state.json" --tenant default --admin-token "$token" --key device/export-smoke --kind device --display-name "Export Smoke" --attributes-json '{"serial":"EXPORT-SMOKE"}' --tag prod --source smoke >/dev/null
