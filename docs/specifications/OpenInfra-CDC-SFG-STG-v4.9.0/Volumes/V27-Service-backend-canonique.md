...
`openinfra-web.service` doit servir un dashboard Bootstrap 5 complet basé sur le modèle Dashboard officiel et sur un header principal unique adapté à OpenInfra. Les composants visibles doivent permettre le pilotage réel des domaines exposés par l'API backend : Dashboard, RSOT (Ressource Source of Truth), IPAM, DCIM, Discovery, Sécurité/RBAC, Audit et Runtime.
