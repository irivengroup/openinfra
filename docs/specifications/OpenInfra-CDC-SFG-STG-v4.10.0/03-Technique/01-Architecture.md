...
Architecture stateless API, workers, PostgreSQL Cluster, cache, queue, object storage, search et observabilité.
...
## Baseline haute performance v4.9.0

Le runtime Pro/Entreprise est ASGI, stateless et multiprocessus. Les limites de concurrence, backlog, keep-alive et taille de corps sont configurables. Le BFF ne détient aucun accès PostgreSQL. Les pools HTTP et PostgreSQL sont créés au démarrage du worker et fermés proprement au shutdown. Le runtime historique est un mécanisme de rollback temporaire, non une cible de dimensionnement.
