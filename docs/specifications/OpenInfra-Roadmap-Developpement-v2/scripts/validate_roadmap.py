...
alignment = read_csv("14-alignement-cdc-v4.8.1.csv")
installers = read_csv("16-plan-installateurs.csv")
pg = read_csv("17-plan-migration-pgdata-lvm.csv")
if len(phases) < 18:
    raise SystemExit(f"Expected at least 18 phases, got {len(phases)}")
if len(epics) < 100:
    raise SystemExit(f"Expected at least 100 epics, got {len(epics)}")
if len(gates) < 8:
    raise SystemExit(f"Expected at least 8 gates, got {len(gates)}")
if len(tests) < 20:
    raise SystemExit(f"Expected at least 20 tests, got {len(tests)}")
if len(alignment) < 20:
...
print(
    f"OK: roadmap v2 validated with {len(phases)} phases, {len(epics)} epics, {len(gates)} gates, {len(tests)} tests"
)
