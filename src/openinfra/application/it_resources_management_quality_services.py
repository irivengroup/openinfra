...
class ItrmQualitySeverity(StrEnum):
    INFO = "info"
    WARNING = "warning"
    ERROR = "error"
...
class ItrmQualityIssue:
    severity: ItrmQualitySeverity
    code: str
    field: str
    message: str
...
class ItrmQualitySummary:
    ...
    rejected: int
    average_score: float
    reports: tuple[ItrmQualityReport, ...]
    evaluated_at: datetime
...
@dataclass(frozen=True, slots=True)
class EvaluateItrmObjectQualityCommand:
    tenant_id: str
    admin_token: str
    key: str
...
@dataclass(frozen=True, slots=True)
class ItrmQualitySummaryCommand:
    tenant_id: str
    admin_token: str
    limit: int = 100
...
class ITResourcesManagementQualityService:
    ...
    def evaluate_object(self, command: EvaluateItrmObjectQualityCommand) -> dict[str, object]:
    ...
    def summarize(self, command: ItrmQualitySummaryCommand) -> ItrmQualitySummary:
        tenant_id = TenantId.from_value(command.tenant_id)
        principal = self._security_service.authenticate_token(
            AuthenticateTokenCommand(
                tenant_id.value,
    ...
    def _certification_status(
        ...
        if any(issue.severity is ItrmQualitySeverity.ERROR for issue in issues) or score < 70:
            return RiCertificationStatus.REJECTED
        if any(issue.severity is ItrmQualitySeverity.WARNING for issue in issues) or score < 90:
...
# Backward-compatible import aliases for integrations compiled against OpenInfra <= 0.29.18.
RiQualitySeverity = ItrmQualitySeverity
RiQualityIssue = ItrmQualityIssue
RiQualityReport = ItrmQualityReport
RiQualitySummary = ItrmQualitySummary
EvaluateRiObjectQualityCommand = EvaluateItrmObjectQualityCommand
RiQualitySummaryCommand = ItrmQualitySummaryCommand
RessourcesInventoryQualityService = ITResourcesManagementQualityService
