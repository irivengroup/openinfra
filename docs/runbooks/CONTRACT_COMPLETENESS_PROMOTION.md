...
GATE-14 qualifie la complétude contractuelle d’OpenInfra 0.34.19. Il ne transforme pas une preuve documentaire ou une qualification à exécuter sur infrastructure réelle en validation fonctionnelle complète.
...
openinfra-gate14 \
  --project-root . \
  --candidate-id openinfra-0.34.19-candidate \
...
4. `PYTEST-AUTOMATION` : 30 preuves automatisées et 43 sélecteurs pytest résolus.
5. `EVIDENCE-CLASSIFICATION` : 589 preuves partielles, 48 externes, 74 fichiers distincts et aucune exigence N1 non classifiée.
