...
class FrontendContractValidator:
    ...
    def _validate_static_operation_catalog(
        ...
        if len(operation_ids) != 288:
            raise FrontendValidationError(
                f"runtime domain catalog must expose 288 operations, found {len(operation_ids)}"
            )
        duplicates = sorted(
            operation_id
    ...
    def _validate_static_assets(self) -> tuple[str, ...]:
        ...
        required_runtime_fragments = (
            ...
            "--openinfra-text-primary: #001b41",
            "--openinfra-text-secondary: #234f7d",
            "--openinfra-text-muted: #315d8a",
            "--openinfra-text-subtle: #3d648d",
            "#24d8ab",
