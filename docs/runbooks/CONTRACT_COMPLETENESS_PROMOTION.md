...
GATE-14 qualifie la complétude contractuelle d’OpenInfra 0.34.7. Il ne transforme pas une preuve documentaire ou une qualification à exécuter sur infrastructure réelle en validation fonctionnelle complète.
...
openinfra-gate14 \
  --project-root . \
  --candidate-id openinfra-0.34.7-candidate \
...
4. `PYTEST-AUTOMATION` : 19 preuves automatisées et 27 sélecteurs pytest résolus.
5. `EVIDENCE-CLASSIFICATION` : 600 preuves partielles, 48 externes, 54 fichiers distincts et aucune exigence N1 non classifiée.
