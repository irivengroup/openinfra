# Cahier des charges entreprise — OpenInfra
## Source of Truth, DCIM, ITAM, Discovery, Dependency Mapping et IPAM avancé, sans fonction ITSM intégrée
...
**Niveau d’exigence :** entreprise / production critique / forte volumétrie  
...
OpenInfra doit être une plateforme open source d’entreprise jouant le rôle de **référentiel central d’infrastructure** et de **Source of Truth** pour les actifs physiques, virtuels, cloud, réseau, applicatifs, logiciels, IP, dépendances, localisations, contrats et capacités.
...
Créer une solution open source professionnelle permettant à une entreprise de disposer d’un référentiel unique, fiable, historisé, interrogeable et exploitable pour piloter son infrastructure IT, ses datacenters, ses réseaux, ses adresses IP, ses dépendances applicatives et ses actifs.
...
### 1.3 Objectifs de conformité entreprise
...
| SAML2 | Fédération d’identité entreprise |
...
Ces références ne transforment pas OpenInfra en solution certifiée par défaut. Elles imposent un niveau de conception, de documentation, de vérification et d’exploitation conforme aux pratiques attendues en entreprise.
...
| Terme | Définition |
|---|---|
| Source of Truth | Référentiel considéré comme source fiable et prioritaire d’une information |
...
|---|---|
| Source of Truth | Objets, relations, statuts, propriétaires, historique, confiance, preuve |
...
[Domain Services]      [Async Job Platform]       [Integration Layer]
 |                          |                            |
 |-- Source of Truth         |-- Discovery Jobs           |-- ITSM
...
| `sot` | objets de référence, relations, statuts, historisation |
...
Aucune donnée de Source of Truth ne doit être écrasée silencieusement par la découverte. Un conflit doit être créé lorsque :
...
| `sot` | objets de référence, relations, statuts |
...
### Phase 2 — Source of Truth et DCIM
...
OpenInfra doit être une solution open source d’entreprise, robuste et complète, couvrant Source of Truth, DCIM, ITAM, SAM, Discovery, Dependency Mapping et IPAM avancé, sans intégrer de fonction ITSM native.
