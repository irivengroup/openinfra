...
PYTHONPATH=src:. python scripts/assemble_scaleout_promotion_evidence.py \
  --candidate-id openinfra-0.34.24-enterprise-scaleout \
  ...
  --release-packaging build/input/packaging/openinfra-0.34.24-release-manifest.json \
  --ga-go-no-go build/input/ga/openinfra-0.34.24-go-no-go.json \
