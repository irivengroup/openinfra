...
def test_access_policy_validation_matching_and_serialization_edges() -> None:
    ...
    assert AccessRequestContext.create(tenant, Permission.IPAM_ALLOCATE).attributes() == {
        "site_code": None,
        "environment": None,
    }
    assert (
        AccessRequestContext.create(
            tenant, Permission.IPAM_ALLOCATE, site_code="par1", environment="PROD"
        ).environment
        == "prod"
    )
    ...
    with pytest.raises(ValidationError):
        AccessPolicyRule.create(
            tenant, "rule-a", Permission.IPAM_ALLOCATE, "allow", subjects=("bad subject!",)
        )
    with pytest.raises(ValidationError):
        AccessPolicyRule.create(
            tenant, "rule-a", Permission.IPAM_ALLOCATE, "allow", environments=("bad env!",)
        )

    default_rule = AccessPolicyRule.create(
        tenant, "default-subject", Permission.IPAM_ALLOCATE, "allow", subjects=(), roles=()
    )
    ...
    restored = AccessPolicyRule.restore(
        ...
        tenant,
        "role-rule",
        Permission.IPAM_ALLOCATE.value,
        "deny",
        (),
        ("ipam:operator",),
        ("*",),
    ...
    principal = AuthenticatedPrincipal(
        tenant,
        "operator",
        (SecurityRole.from_value("ipam:operator"),),
        frozenset({Permission.IPAM_ALLOCATE}),
    )
    assert restored.applies_to_principal(principal) is True
    assert (
        restored.matches_context(
            AccessRequestContext.create(
                tenant, Permission.IPAM_ALLOCATE, site_code="PAR1", environment="prod"
            )
        )
    ...
    assert (
        restored.matches_context(
            AccessRequestContext.create(
                tenant, Permission.SCHEMA_READ, site_code="PAR1", environment="prod"
            )
        )
        is False
    )
    ...
    site_rule = AccessPolicyRule.create(
        tenant,
        "site-only",
    ...
    assert (
        site_rule.matches_context(
            AccessRequestContext.create(
                tenant, Permission.IPAM_ALLOCATE, site_code=None, environment="prod"
            )
        )
        is False
    )
    assert (
        site_rule.matches_context(
            AccessRequestContext.create(
                tenant, Permission.IPAM_ALLOCATE, site_code="PAR1", environment=None
            )
        )
        is False
    )
