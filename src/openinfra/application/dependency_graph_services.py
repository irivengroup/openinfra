...
@dataclass(frozen=True, slots=True)
class AnalyzeChangeImpactCommand:
    tenant_id: str
    admin_token: str
    root_key: str
    direction: str = GraphDirection.INCOMING.value
    ...
    relation_types: tuple[str, ...] = ()
    as_of: str | datetime | None = None
    business_service_kinds: tuple[str, ...] = (
        "application",
        "service",
        "software-service",
        "database",
    )
    business_service_resource_types: tuple[str, ...] = (
        "application",
        "service",
        "api-service",
        "web-service",
...
class DependencyGraphService:
    ...
    def analyze_change_impact(
        self, command: AnalyzeChangeImpactCommand
    ) -> DependencyChangeImpactReport:
        tenant_id, subject = self._authorize(command.tenant_id, command.admin_token)
        direction = GraphDirection.from_value(command.direction)
        max_depth, max_nodes = self._limits(command.max_depth, command.max_nodes)
        relation_types = self._relation_types(command.relation_types)
        as_of = self._datetime(command.as_of)
        business_kinds = self._filter_values(
            command.business_service_kinds, "business service kind"
        )
        business_types = self._filter_values(
            command.business_service_resource_types, "business service resource type"
        )
        ...
        with self._transaction_manager.begin() as unit_of_work:
            ...
            impacted_nodes = tuple(node for node in graph.nodes if node.key != graph.root_key)
            business_services = tuple(
                sorted(
                    (
                        node
                        for node in impacted_nodes
                        if node.kind in business_kinds or node.resource_type in business_types
                    ),
                    key=lambda node: (node.depth, node.key),
                )
            )
            analysis = self._analyze_spof_graph(graph)
            business_indexes = {
                analysis.indexes[node.key]
                for node in business_services
                if node.key in analysis.indexes
            }
            root_index = analysis.indexes[graph.root_key]
            risks: list[DependencyCriticalRisk] = []
            for index, node in enumerate(analysis.nodes):
                if index == root_index:
                    continue
                affected_business = tuple(
                    sorted(
                        analysis.nodes[business_index].key
                        for business_index in business_indexes
                        if analysis.dominators[business_index] & (1 << index)
                        and business_index != index
                    )
                )
                if not affected_business:
                    continue
                affected_nodes = analysis.affected_counts[index]
                direct_affected = sum(
                    1
                    for successor in analysis.successors[index]
                    if analysis.dominators[successor] & (1 << index)
                )
                risk_level = (
                    "critical"
                    if len(affected_business) >= 5 or affected_nodes >= 20
                    else "high"
                    if len(affected_business) >= 2 or affected_nodes >= 5
                    else "medium"
                )
                risks.append(
                    DependencyCriticalRisk(
                        node=node,
                        affected_business_service_count=len(affected_business),
                        affected_business_service_keys=affected_business[:sample_limit],
                        affected_node_count=affected_nodes,
                        direct_affected_count=direct_affected,
                        risk_level=risk_level,
                    )
                )
            risks.sort(
                key=lambda risk: (
                    -risk.affected_business_service_count,
                    -risk.affected_node_count,
                    risk.node.key,
                )
            )
            report = DependencyChangeImpactReport(
                ...
                critical_dependencies=tuple(risks),
                root_spof_risk=bool(business_services),
                max_depth_reached=graph.max_depth_reached,
                truncated=graph.truncated,
                edges=graph.edges,
            )
            self._audit_repository.append(
                AuditEvent.record(
                    tenant_id=tenant_id,
                    actor=subject,
                    action="graph.change-impact.analyze",
                    target_type="source_object",
                    target_id=graph.root_key,
                    metadata={
                        "direction": direction.value,
                        ...
                        "critical_dependency_count": len(risks),
                        "root_spof_risk": report.root_spof_risk,
