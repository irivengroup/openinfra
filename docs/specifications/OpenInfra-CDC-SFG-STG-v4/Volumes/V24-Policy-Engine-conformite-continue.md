# Volume 24 — Policy Engine et conformité continue
...
Appliquer des règles de conformité transverses sur la donnée, l’architecture, la sécurité, l’IPAM, le réseau, le cloud, Kubernetes, les licences et les coûts.
...
- Vérifier que toute application production possède propriétaire, criticité, flux déclarés et certificats suivis.
- Détecter tous les subnets sans zone DNS inverse dans une VRF de production.
- Simuler une nouvelle politique de tagging cloud avant application réelle.
...
- Règles trop nombreuses entraînant bruit opérationnel
- Politiques mal scindées entre conformité et automatisation

## 11. Exigences vérifiables du volume

| ID | Priorité | Type | Exigence | Vérification | Acceptation |
|---|---:|---|---|---|---|
| REQ-00474 | N1 | Fonctionnelle | OpenInfra doit fournir le module Policy Engine et conformité continue avec APIs, UI, audit, RBAC, imports/exports contrôlés et traçabilité complète. | Test fonctionnel bout-en-bout, revue API/UI et vérification audit. | Le module Policy Engine et conformité continue est utilisable par API et UI avec droits, audit et critères d’acceptation vérifiés. |
| REQ-00475 | N1 | Fonctionnelle | Le module Policy Engine et conformité continue doit être strictement séparé du périmètre ITSM et ne doit pas créer de ticket, incident, demande ou changement natif. | Revue fonctionnelle et tests négatifs sur les workflows. | Aucune capacité de ticketing natif n’est exposée dans ce module. |
| REQ-00476 | N1 | Données | Le modèle de données du module Policy Engine et conformité continue doit utiliser des entités typées, contraintes d’intégrité, propriétaires, tenant, statut, historique et audit. | Revue dictionnaire, tests migrations et tests contraintes. | Chaque entité critique possède clé stable, contraintes, audit et règles de rétention. |
| REQ-00477 | N1 | Performance | Les données volumineuses du module Policy Engine et conformité continue doivent être partitionnées, paginées, indexées et exploitées sans scan complet non maîtrisé. | Analyse DDL, tests EXPLAIN, benchmark API. | Les requêtes critiques utilisent les index attendus et les exports massifs sont asynchrones. |
| REQ-00478 | N1 | Sécurité | Le module Policy Engine et conformité continue doit appliquer RBAC/ABAC, isolation tenant, masquage des données sensibles et audit immuable des opérations critiques. | Tests RBAC/ABAC, tests audit et tests sécurité. | Un utilisateur sans droit ne peut ni lire ni modifier les ressources hors périmètre. |
| REQ-00479 | N1 | Résilience | Les traitements longs du module Policy Engine et conformité continue doivent être asynchrones, idempotents, relançables, annulables si possible et suivis par métriques. | Tests worker, crash recovery, dead-letter queue et reprise. | Un crash worker ne perd aucun job validé et l’état final reste cohérent. |
| REQ-00480 | N2 | Observabilité | Le module Policy Engine et conformité continue doit exposer métriques, logs structurés, traces et événements métier corrélables. | Test observabilité et contrôle dashboards. | Chaque opération critique produit log, métrique, trace et événement corrélés. |
| REQ-00481 | N1 | API | Les APIs du module Policy Engine et conformité continue doivent supporter pagination cursor-based, filtres sélectifs, tri indexé, OpenAPI et webhooks métier. | Tests API contractuels et tests de charge. | Les endpoints refusent les lectures volumineuses non filtrées et les contrats API sont validés. |
| REQ-00482 | N1 | Audit | Toute modification métier du module Policy Engine et conformité continue doit conserver ancien état, nouvel état, acteur, source, corrélation et preuve. | Tests audit et relecture historique. | Un changement peut être reconstitué avec preuve et horodatage. |
| REQ-00483 | N2 | Intégration | Le module Policy Engine et conformité continue doit publier des événements temps réel et consommer les données des autres domaines OpenInfra sans duplication incohérente. | Test event bus et test cohérence inter-domaines. | Les relations transverses sont référencées par identifiants stables et contrôles d’intégrité. |
| REQ-00484 | N1 | Fonctionnelle | Le module Policy Engine et conformité continue doit fournir au moins les capacités suivantes : moteur de règles déclaratif versionné; politiques par tenant, site, environnement, criticité, domaine et objet; évaluation continue et planifiée; exceptions avec justification, expiration et propriétaire. | Scénarios nominaux API/UI et tests fonctionnels. | Les quatre capacités prioritaires sont démontrées avec données représentatives. |
| REQ-00485 | N2 | Fonctionnelle | Le module Policy Engine et conformité continue doit fournir les capacités avancées suivantes : preuves d’audit attachées aux constats; score conformité par domaine, application, tenant et site; webhooks et intégrations SIEM/GRC externes; mode simulation avant activation d’une politique. | Tests fonctionnels et revue documentaire. | Les capacités avancées sont documentées, sécurisées et traçables. |
| REQ-00486 | N1 | Qualité | Les données du module Policy Engine et conformité continue doivent être vérifiées par règles qualité, exceptions justifiées et rapports de conformité. | Tests règles qualité et tests exceptions. | Les écarts sont détectés, priorisés, historisés et exportables. |
| REQ-00487 | N2 | Exploitation | Le module Policy Engine et conformité continue doit disposer de runbooks d’exploitation, sauvegarde, purge, restauration et support diagnostic. | Revue runbooks et test restauration ciblée. | Les procédures sont exécutables et reliées aux métriques de supervision. |
| REQ-00488 | N1 | Acceptation | Le module Policy Engine et conformité continue doit disposer de critères d’acceptation mesurables, de tests automatisés et d’une traçabilité exigence → cas d’usage → test. | Contrôle matrice de traçabilité. | Aucune exigence N1 du module n’est orpheline. |

## 12. Traçabilité

Toutes les exigences ci-dessus sont rattachées dans `11-Matrices/Traceabilite.csv` au cas d’usage `UC-0022` et aux tests `TST-REQ-*`. Les entités ajoutées sont présentes dans `04-Donnees/Dictionnaire.csv`.
